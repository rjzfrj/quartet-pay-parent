...
<article class="page-container">
	<form  action="${baseURL}/coupon/product/add"  method="post" id="pageform" class="form form-horizontal" >
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>商品名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input name="productName" id="productName" type="text" class="input-text" style="width:300px;"/>
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<div class="report-file">
						<span>上传文件…</span>
						<input tabindex="3" size="3" id="myfiles" name="myfiles" type="file"  class="file-prew" type="file">
				</div>
				<input type="button" class="btn btn-primary radius" value="上传图片" onclick="ajaxFileUpload()"/>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>商品标题：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input name="goodsTitle" class="input-text" style="width:300px;" maxlength="30"  size="30" autocomplete="off"/>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">商品描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="goodsBody" cols="" rows="" class="textarea"  ></textarea>
...
<!--/请在上方写此页面业务相关的脚本-->
	<script type="text/javascript">
	function checkData() {
		var fileDir = $("#myfiles").val();
		var suffix = fileDir.substr(fileDir.lastIndexOf("."));
		if ("" == fileDir) {
//			alert("选择需要导入的Excel文件！");
			window.wxc.xcConfirm( "选择需要上传的图片！", window.wxc.xcConfirm.typeEnum.info);
			return false;
		}
		var bzsuffix=suffix.toLocaleLowerCase();
		if (".jpg" != bzsuffix && ".gif" != bzsuffix && ".bmp" != bzsuffix) {
			window.wxc.xcConfirm( "选择jpg,gif,bmp格式的图片上传！", window.wxc.xcConfirm.typeEnum.info);
			return false;
		}
		return true;
	}
