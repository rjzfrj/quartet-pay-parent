...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 交易管理<span class="c-gray en">&gt;</span> 调单处理管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
		<form id="pageform" title="workList" action="${baseURL}/tran/transInvestig/list" method="post">
			...
			<div>
				...
				<select name="orderTypeId" class="select" style="width: 100px;">
					...
					<c:forEach var="item" items="${orderTypeList }">
						<option value="${item.value1 }" <c:if test="${transInvestig.orderTypeId==item.value1 }">selected="selected" </c:if>  >${item.name }</option>
					</c:forEach>
				</select>&nbsp;
				调单类型：
				<select name="type" class="select" style="width: 100px;">
					...
					<c:forEach var="item" items="${transInvestigTypeList }">
						<option value="${item.value1 }" <c:if test="${transInvestig.type==item.value1 }">selected="selected" </c:if>  >${item.name }</option>
					</c:forEach>
				</select>&nbsp;
				调单状态：
				<select name="status" class="select" style="width: 100px;">
					<option value="">全部</option>
					<c:forEach var="item" items="${transInvestigStatusList }">
						<option value="${item.value1 }"  <c:if test="${transInvestig.status==item.value1 }">selected="selected" </c:if>  >${item.name }</option>
					</c:forEach>
				</select>&nbsp;
				流水号：
				<input class="input-text" style="width: 150px;" type="text" name="investigId" value="${transInvestig.investigId}" size="30"  />&nbsp;
				商户号： 
				<input class="input-text" style="width: 150px;" type="text" name="mcode" value="${transInvestig.mcode}" size="30" alt="模糊查询" />&nbsp;
				终端号： 
				<input class="input-text" style="width: 150px;" type="text" name="tcode" value="${transInvestig.tcode}" size="30" alt="模糊查询" />
				<br/>
			</div>
			<div style="margin-top: 10px;">
				所属通道： 
				<input class="input-text" style="width: 150px;" type="text" name="channelId" value="${transInvestig.channelId}" size="30" alt="模糊查询" />&nbsp;
				调单时间：<input class="input-text Wdate" style="width: 150px;" readonly="readonly" name="startDateCreate" value="<fmt:formatDate value='${transInvestig.startDateCreate}' pattern='yyyy-MM-dd HH:mm:ss' />" type="text" 
					...
					--<input class="input-text Wdate" style="width: 150px;" readonly="readonly" name="endDateCreate" value="<fmt:formatDate value='${transInvestig.endDateCreate}' pattern='yyyy-MM-dd HH:mm:ss' />" type="text" 
				...
				&nbsp;
				更新时间：<input class="input-text Wdate" style="width: 150px;" readonly="readonly" name="startDateEdit" value="<fmt:formatDate value='${transInvestig.startDateEdit}' pattern='yyyy-MM-dd HH:mm:ss' />" type="text" 
							onfocus="WdatePicker({dateFmt:'yyyy-M-d H:mm:ss',qsEnabled:true,quickSel:['%y-%M-{%d-4} 00:00:00','%y-%M-{%d-4} 00:00:00','%y-%M-{%d-3} 00:00:00','%y-%M-{%d-2 } 00:00:00','%y-%M-%d 00:00:00']})"/>
					--<input class="input-text Wdate" style="width: 150px;" readonly="readonly" name="endDateEdit" value="<fmt:formatDate value='${transInvestig.endDateEdit}' pattern='yyyy-MM-dd HH:mm:ss' />" type="text" 
						onfocus="WdatePicker({dateFmt:'yyyy-M-d H:mm:ss',qsEnabled:true,quickSel:['%y-%M-{%d-4} 23:59:59','%y-%M-{%d-3} 23:59:59','%y-%M-{%d-2} 23:59:59','%y-%M-{%d-1} 23:59:59','%y-%M-%d 23:59:59']})"/>
				&nbsp;&nbsp;
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="${item.investigId}"  name="ids"></td>
				<td>${item.investigId }</td>
				<td>${item.tcode}</td>
				<td>${item.mcode}</td>
				<td>${item.merName}</td>
				<td>
					<c:forEach var="ty" items="${orderTypeList }">
						<c:if test="${item.orderTypeId==ty.value1 }"> ${ty.name }</c:if>
					</c:forEach>
				</td>
				<td>
					<c:forEach var="ty" items="${transInvestigTypeList }">
						<c:if test="${item.type==ty.value1 }"> ${ty.name }</c:if>
					</c:forEach>
				</td>
				<td>
				...
				<td>
					<c:forEach  var="sositem" items="${transInvestigStatusList}">
