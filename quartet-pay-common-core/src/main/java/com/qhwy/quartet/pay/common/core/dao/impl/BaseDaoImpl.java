package com.qhwy.quartet.pay.common.core.dao.impl;
...
import com.qhwy.quartet.pay.common.core.exception.BizException;
...
public class BaseDaoImpl<T> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public int insert(List<T> list) {
		...
		if (result <= 0) {
			throw BizException.DB_INSERT_RESULT_0.newInstance("数据库操作,batchInsert返回0.{%s}",
	...
	public int update(T entity) {
		...
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,updateByPrimaryKey返回0.{%s}",
	...
	public int updateByPrimaryKeySelective(T entity) {
		...
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,updateByPrimaryKeySelective返回0.{%s}",
					getStatement(SQL_UPDATE_BY_ID));
		}
		return result;
	}
	...
	public int update(List<T> list) {
		...
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,batchUpdateByIds返回0.{%s}",
	 ...
	 * 根据id批量更新数据.需指定sqlId
	 */
	public int update(List<T> list,String sqlKey) {
		if (list.isEmpty() || list.size() <= 0) {
			return 0;
		}
		int result = sessionTemplate.update(getStatement(sqlKey), list);
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,batchUpdateByIds返回0.{%s}",
					getStatement(SQL_BATCH_UPDATE_BY_IDS));
		}
	...
	public int update(Map<String, Object> paramMap) {
		...
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,batchUpdateByColumn返回0.{%s}",
					getStatement(SQL_BATCH_UPDATE_BY_COLUMN));
		}
		return result;
	}
	/**
	 * 根据column批量更新数据.需指定sqlId
	 */
	public int update(Map<String, Object> paramMap,String sqlKey) {
		if (paramMap == null) {
			return 0;
		}
		int result = sessionTemplate.update(getStatement(sqlKey), paramMap);
		if (result <= 0) {
			throw BizException.DB_UPDATE_RESULT_0.newInstance("数据库操作,batchUpdateByColumn返回0.{%s}",
					getStatement(SQL_BATCH_UPDATE_BY_COLUMN));
		}
	...
	/**
	 * 根据id批量删除数据. 需要指定sqlId
	 */
	public int delete(List<T> list,String sqlKey) {
		if (list.isEmpty() || list.size() <= 0) {
			return 0;
		} else {
			return (int) sessionTemplate.delete(getStatement(sqlKey), list);
		}
	}
	...
	/**
	 * 根据column批量删除数据. 需指定sqlId 方便具体业务特殊需求自定义sql使用
	 */
	public int delete(Map<String, Object> paramMap,String sqlKey) {
		if (paramMap == null) {
			return 0;
		} else {
			return (int) sessionTemplate.delete(getStatement(sqlKey), paramMap);
		}
	}
	...
	public PageBean listPage(PageParam pageParam, Map<String, Object> paramMap,String sqlConutKey,String sqlListKey) {
		...
		// 统计总记录数
		Long totalCount = sessionTemplate.selectOne(getStatement(sqlConutKey), paramMap);
		...
		// 获取分页数据集
		RowBounds rowBounds = new RowBounds(pageFist, pageParam.getNumPerPage());
		List<Object> list = sessionTemplate.selectList(getStatement(sqlListKey), paramMap, rowBounds);
	...
	public List<T> selectEntityList(String sqlKey, T entity) {
		return sessionTemplate.selectList(getStatement(sqlKey), entity);
	}
	...
	public T selectEntity(T entity) {
		...
		return sessionTemplate.selectOne(getStatement(SQL_LIST_BY_COLUMN), entity);
	}

	@Override
	public T selectEntity(String sqlKey, T entity) {
		if (entity == null || sqlKey == null) {
			return null;
		}
		return sessionTemplate.selectOne(getStatement(sqlKey), entity);
	}
