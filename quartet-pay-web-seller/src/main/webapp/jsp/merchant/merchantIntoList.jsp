...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i>商户入件费率列表<span class="c-gray en">&gt;</span> 商户费率管理<span class="c-gray en">&gt;</span> 菜单管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
		<form id="pageform" title="workList" action="${baseURL}/merchant/freerate/list" method="post">
						...
						商户号
						<input type="text" name="mcode" value="${freerate.mcode}" class="input-text" style="width:250px"  />&nbsp;
						创建时间：
						<input type="text" name="remark" value="${freerate.remark}" class="input-text" style="width:200px" />--
					    <input type="text" name="remark" value="${freerate.remark}" class="input-text" style="width:200px" />
						<%-- 
						友好描述：
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="${item.tcIntoId}" name="ids"></td>
				<td>${st.index+1}</td>
				<td>${item.orgId}</td>
				<td>${item.mcode}</td> 
			...
			<%-- 	<td>${item.createTime}</td> --%>
				<td>
				 <c:if test="${item.status==0}">申请失败</c:if>
				 <c:if test="${item.status==1}">申请成功</c:if>
				 <c:if test="${item.status==2}">等待银联处理</c:if>
				 <c:if test="${item.status==3}">未上送</c:if>
				 <c:if test="${item.status==4}"> 校验失败</c:if>
				</td>
				
				<td>
				 <c:if test="${item.roleId==1}">混合 信用卡费率 储蓄卡封顶</c:if>
				 <c:if test="${item.status==2}">费率(特殊) </c:if>
				</td>
				<td>${item.userName}</td>
				<td>${item.simpleMoney}</td>
				<td>${item.channelId}</td>
				<td>${item.payChannelType}</td>
				<td>
				 <c:if test="${item.roleId==1}">T1</c:if>
				 <c:if test="${item.status==2}">T0 </c:if>
				</td>
				<td>
				 <c:if test="${item.invokePattern==1}">直连模式</c:if>
				 <c:if test="${item.invokePattern==2}"> 直连改造模式 </c:if>
				 <c:if test="${item.invokePattern==3}"> sdk模式</c:if>
				 <c:if test="${item.invokePattern==4}">直清模式 </c:if>
				</td>
				<td class="td-manage">
