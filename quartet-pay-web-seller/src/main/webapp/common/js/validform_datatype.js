...
(function(){
	if($.Datatype){
		$.extend($.Tipmsg.w,{
			...
			"idcard":"身份证号码不对！",
			"num2":"请填写数值！",
			"rebateDateVaildJinsai":"开始日期不能大于结束日期！"
		});
		...
		$.extend($.Datatype,{
			...
			*/
			...
			"rebateDateVaildJinsai":function(gets,obj,curform,datatype){				
				var minim=$("#startRebateDate").val(),
					maxim=$("#endRebateDate").val();
					
				if((null == minim || "" == minim) && (null != maxim || "" != maxim)){
					return true;
				}else if((null != minim || "" != minim) && (null == maxim || "" == maxim)){
					return true;
				}
			...
			"empty" : function(gets,obj,curform,datatype){
				if(gets.replace(/\s/g,'')==''){
					return true;
				}
				return false;
