...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 菜单管理<span class="c-gray en">&gt;</span>商户管理 <span class="c-gray en">&gt;</span> <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
		<form id="pageform" title="workList" action="${baseURL}/merchant/list" method="post">
						...
						商户号
						<input type="text" name="mcode" value="${pmsMerchant.mcode}" class="input-text" style="width:150px"  />&nbsp;
						商户名
						<input type="text" name="merName" value="${pmsMerchant.merName}" class="input-text" style="width:150px"  />&nbsp;
						机构号
						<input type="text" name="orgId" value="${pmsMerchant.orgId}" class="input-text" style="width:150px"  />&nbsp;
						商户状态：
						<select name="status">
				       ...
				       <input name="endDateCreate"  type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:159px;">
					            ...
					            修改时间：
				      <input name="startDateEdit"   type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d 23:00:00'})" class="input-text Wdate" style="width:159px;"/>
				       -
				        <input name="endDateEdit"  type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss', minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:159px;">
	...
	<span class="l">
		...
		<a href="javascript:;" onclick="openPage('添加商户','${baseURL}/merchant/addUI')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加商户</a>
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>${st.index+1}</td>
					...
					<td>${item.merName}</td>
					<td>
					<c:if test="${item.status==1}"><span class="label label-success radius">正常</span></c:if>
					<c:if test="${item.status==2}"><span class="label radius">停用</span></c:if>
				...
				<td class="td-manage">
				<a title="编辑" href="javascript:;" onclick="show('编辑','${baseURL}/merchant/toEditUI?mcode=${item.mcode}','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
